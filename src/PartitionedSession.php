...
class PartitionedSession implements SessionInterface, SessionManagerInterface
{
    ...
    public function startPartitioned(ResponseInterface $response): ResponseInterface
    {
        if ($this->isStarted()) {
        ...
        $cookie = SetCookie::create(session_name())
            ...
            ->withMaxAge($current['lifetime'])
            ->withPath($current['path'])
            ->withDomain($current['domain'])
            ->withSameSite(SameSite::fromString($current['samesite']));
        if ($current['httponly']) {
